...
int main()
{
    ...
    Vertex s;
    s.id = 0;
    DAG_LONGEST_PATH(G, s, Topological_Vector);*/
...
vector<int> DAG_LONGEST_PATH(Graph G, Vertex s, vector<int> Topological_vector)
{
    ...
    int i = Max_leaf;
    int j = 1; 
    while(Vertex_Array[i].pid != -2)
    {
        LPATH[j] = i;
        i = Vertex_Array[i].pid;
        j++;
    }
    LPATH[j] = Vertex_Array[i].pid;


    /*for(int i = 0; i < G.V; i++)
        cout << Vertex_Array[i].id << ")<--(" << Vertex_Array[i].pid << endl;*/
    /*
    ...
    cout << "DAG lpath from source:" << s.id << endl;
    for(int i = 0; i < G.V; i++)
        cout << "(" << LPATH[i] << ")<--" ;
    cout << endl;
    return LPATH;
