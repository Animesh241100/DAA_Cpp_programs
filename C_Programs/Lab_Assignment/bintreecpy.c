...
{ 
	static int preIndex = 0;  //preIndex is the index vlue of a data in preorder traversal

	if (inStrt > inEnd) 
		return NULL; 

	struct node* root = newNode(pre[preIndex]); //making a node from data associated with current preIndex
	preIndex = preIndex + 1;
